...
<!-- ## If you decide to build something like this, here is an incomplete list of things to consider: -->
## If that's something you want to do, here is an incomplete list of things to consider:

- **PCIe**
    - PCIe is a [bus](https://en.wikipedia.org/wiki/Bus_(computing)) meaning that multiple devices can share the same bandwidth, one PCIe lane can have multiple devices connected, those devices then share the bandwidth if multiple device want to communicate at the same time
...
- **cooling/ventilation**
    ...
    - [here](https://twitter.com/mkinyugo/status/1753720743706153034) is an example for the expected performance loss a 4090 experiences with different power limits (keep in mind this might not be exactly the same on your particular workload on your particular hardware)
